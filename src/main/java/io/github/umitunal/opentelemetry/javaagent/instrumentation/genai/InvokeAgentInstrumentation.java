...
/** Wraps every {@code @InvokeAgent}-annotated method in an {@code invoke_agent} span. */
final class InvokeAgentInstrumentation implements TypeInstrumentation {
  ...
  public void transform(TypeTransformer transformer) {
    transformer.applyAdviceToMethod(
        isAnnotatedWith(named(ANNOTATION)), getClass().getName() + "$InvokeAgentAdvice");
  }

  @SuppressWarnings("unused")
  public static final class InvokeAgentAdvice {
    private InvokeAgentAdvice() {}

    @Advice.OnMethodEnter(suppress = Throwable.class, inline = false)
    public static Object onEnter(@Advice.Origin Method method) {
      return AgentSpans.startAgent(method);
    }
